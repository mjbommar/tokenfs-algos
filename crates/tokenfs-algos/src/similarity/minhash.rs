...
mod tests {
    ...
    fn build_byte_table_from_seeds_into_matches_boxed_reference() {
        macro_rules! check_k_heap {
            ($k:literal) => {{
                ...
                use core::mem::MaybeUninit;
                let mut uninit: Box<MaybeUninit<[[u64; $k]; kernels_gather::TABLE_ROWS]>> =
                    Box::new_uninit();
    ...
    fn build_byte_table_from_seeds_into_handles_minimal_k8() {
        ...
        for cell in &table[0] {
            assert_eq!(*cell, crate::hash::mix_word(0));
        }
        for cell in &table[0xFF] {
            assert_eq!(*cell, crate::hash::mix_word(0xFF));
        }
    }
}
