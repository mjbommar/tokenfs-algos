...
// Test code — panic on None is the desired failure mode.
// The `similarity::kernels::*` paths used in some sub-tests below are
// deprecated re-exports of `vector::kernels::*` (v0.2 module split); the
// parity coverage they provide is still load-bearing — the new
// `vector::*` surface gets dedicated parity assertions added in the same
// file, and the deprecated paths are exercised here to prove the shim
